        ...
        <form onSubmit={handleEnhance} className="flex flex-col items-center">
          <div className="w-full max-w-[400px]">
            <button
              type="submit"
              disabled={isLoading}
              className={`w-full bg-gradient-to-r from-pink-600 to-purple-600 px-6 py-3 rounded-xl font-semibold transition-all duration-300 flex items-center justify-center
                ${isLoading ? "opacity-70 cursor-not-allowed" : "hover:from-pink-700 hover:to-purple-700"}
              `}
            >
              {isLoading ? (
                <>
          ...
          {error && (
            <p className="text-red-400 text-sm mt-3 max-w-[400px] text-center">
              {error}
            </p>
          )}
